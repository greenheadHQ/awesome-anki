 ...
 */
export function resolveModelId(provider?: string, model?: string): LLMModelId | undefined {
  if (model && !provider) {
    throw new ValidationError("model을 지정할 때 provider도 함께 지정해야 합니다.");
  }
  ...
  const resolvedModel = model ?? getDefaultModelForProvider(provider as LLMProviderName);
